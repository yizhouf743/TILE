...
void TILERelu4(int32_t s1, int32_t s2, int32_t s3, int32_t s4, int32_t tile_type, float apply_ratio, uint64_t* inArr, uint64_t* outArr, int32_t sf, uint32_t doTruncation){
int64_t base_relu = (((s1 * s2) * s3) * s4);
int padding_image_size = next_pow2(s2 * s3);
int chan_per_ciphertext = 8192 / 2 / padding_image_size;
int break_point = ceil((float) s4 * apply_ratio / chan_per_ciphertext) * chan_per_ciphertext;
int64_t pooled_size;
int64_t rest_size;
if (tile_type == 0){
    pooled_size = floor(base_relu * apply_ratio / 4);
    rest_size = base_relu - pooled_size * 4;
}
else{
    pooled_size = floor(base_relu * apply_ratio / 2);
    rest_size = base_relu - pooled_size * 2;
}
int64_t relu_size = rest_size + pooled_size;
cout << "before and after Relu size: " << base_relu << "/" << relu_size << endl;
uint64_t* reshapedInArr = make_array<uint64_t>(relu_size);
uint64_t* reshapedOutArr = make_array<uint64_t>(relu_size);

// compress input into reshapedInArr
for (uint64_t i1 = 0; i1 < s1; i1++) {
    for (uint64_t i2 = 0; i2 < s2; i2++) {
        for (uint64_t i3 = 0; i3 < s3; i3++) {
            for (uint64_t i4 = 0; i4 < s4; i4++) {
                int64_t linIdx = (((((i1 * s2) * s3) * s4) + ((i2 * s3) * s4)) + (i3 * s4)) + i4;
                if (tile_type == 0 && i4 < break_point) {
                    if (i2 % 2 == 0 && i3 % 2 == 0) {
                        int64_t relu_linIdx = (((((i1 * (s2 / 2)) * (s3 / 2)) * s4) + ((i2 / 2 * (s3 / 2)) * s4)) + ((i3 / 2) * s4)) + i4;
                        if (relu_linIdx < relu_size) {
                            Arr1DIdxRowM(reshapedInArr, relu_size, relu_linIdx) = Arr4DIdxRowM(inArr, s1, s2, s3, s4, i1, i2, i3, i4);
                        }
                    }
                } else if (tile_type == 1 && i4 < break_point) {
                    if (i4 % 2 == 0) {
                        int64_t relu_linIdx = linIdx / 2;
                        if (relu_linIdx < relu_size) {
                            Arr1DIdxRowM(reshapedInArr, relu_size, relu_linIdx) = Arr4DIdxRowM(inArr, s1, s2, s3, s4, i1, i2, i3, i4);
                        }
                    }
                    ...
                    if (linIdx < relu_size) {
                        Arr1DIdxRowM(reshapedInArr, relu_size, linIdx) = Arr4DIdxRowM(inArr, s1, s2, s3, s4, i1, i2, i3, i4);
                    }
                }
            }
        }
...
Relu(relu_size, reshapedInArr, reshapedOutArr, sf, doTruncation);

// recovery shape from reshapedOutArr to outArr
for (uint64_t i1 = 0; i1 < s1; i1++) {
    for (uint64_t i2 = 0; i2 < s2; i2++) {
        for (uint64_t i3 = 0; i3 < s3; i3++) {
            for (uint64_t i4 = 0; i4 < s4; i4++) {
                int64_t linIdx = (((((i1 * s2) * s3) * s4) + ((i2 * s3) * s4)) + (i3 * s4)) + i4;
                if (tile_type == 0 && s4 < break_point) {
                    if (i2 % 2 == 0 && i3 % 2 == 0) {
                        int64_t relu_linIdx = (((((i1 * (s2 / 2)) * (s3 / 2)) * s4) + ((i2 / 2 * (s3 / 2)) * s4)) + ((i3 / 2) * s4)) + i4;
                        if (relu_linIdx < relu_size) {
                            Arr4DIdxRowM(outArr, s1, s2, s3, s4, i1, i2, i3, i4) = Arr1DIdxRowM(reshapedOutArr, relu_size, relu_linIdx);
                            Arr4DIdxRowM(outArr, s1, s2, s3, s4, i1, i2 + 1, i3, i4) = Arr1DIdxRowM(reshapedOutArr, relu_size, relu_linIdx);
                            Arr4DIdxRowM(outArr, s1, s2, s3, s4, i1, i2, i3 + 1, i4) = Arr1DIdxRowM(reshapedOutArr, relu_size, relu_linIdx);
                            Arr4DIdxRowM(outArr, s1, s2, s3, s4, i1, i2 + 1, i3 + 1, i4) = Arr1DIdxRowM(reshapedOutArr, relu_size, relu_linIdx);
                        }
                    }
                } else if (tile_type == 1 && i4 < break_point) {
                    if (i4 % 2 == 0) {
                        ...
                        if (relu_linIdx < relu_size) {
                            Arr4DIdxRowM(outArr, s1, s2, s3, s4, i1, i2, i3, i4) = Arr1DIdxRowM(reshapedOutArr, relu_size, relu_linIdx);
                            Arr4DIdxRowM(outArr, s1, s2, s3, s4, i1, i2, i3, i4 + 1) = Arr1DIdxRowM(reshapedOutArr, relu_size, relu_linIdx);
                        }
                    }
                } else {
                    if (linIdx < base_relu) {
                        Arr4DIdxRowM(outArr, s1, s2, s3, s4, i1, i2, i3, i4) = Arr1DIdxRowM(reshapedOutArr, base_relu, linIdx);
                    }
                }
            }
        }
...
ClearMemSecret1(relu_size, reshapedInArr);
ClearMemSecret1(relu_size, reshapedOutArr);
}
...
string outputIndicesFilePath = "/path/tensor_1.txt";
vector<size_t> inputIndices = {};
vector<size_t> outputIndices;
inputIndices = {};
...
TILEWrapper( (int32_t)1,  (int32_t)32,  (int32_t)32,  (int32_t)64,  (int32_t)3,  (int32_t)3,  (int32_t)64, (int32_t)0, (float)0.0, (int32_t)0, (float).375, (int32_t)1, (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1, tmp44, tmp46, tmp48, inputIndices, outputIndices);
...
TILERelu4( (int32_t)1,  (int32_t)32,  (int32_t)32,  (int32_t)64, (int32_t)0, (float).375, tmp58, tmp60,  kScale, kDoExtractTruncate);
...
string inputIndicesFilePath = "/path/tensor_1.txt";
outputIndicesFilePath = "/path/tensor_2.txt";
...
TILEWrapper( (int32_t)1,  (int32_t)32,  (int32_t)32,  (int32_t)64,  (int32_t)3,  (int32_t)3,  (int32_t)128, (int32_t)0, (float).375, (int32_t)0, (float).3438, (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1, tmp64, tmp66, tmp68, inputIndices, outputIndices);
...
TILERelu4( (int32_t)1,  (int32_t)32,  (int32_t)32,  (int32_t)128, (int32_t)0, (float).3438, tmp77, tmp79,  kScale, kDoExtractTruncate);
...
inputIndicesFilePath = "/path/tensor_2.txt";
outputIndicesFilePath = "/path/tensor_3.txt";
inputIndices = loadIndicesFromTxt(inputIndicesFilePath);
outputIndices = loadIndicesFromTxt(outputIndicesFilePath);

uint64_t* tmp87 = make_array<uint64_t>( (int32_t)1,  (int32_t)32,  (int32_t)32,  (int32_t)128);
TILEWrapper( (int32_t)1,  (int32_t)32,  (int32_t)32,  (int32_t)128,  (int32_t)3,  (int32_t)3,  (int32_t)128,  (int32_t)0, (float).3438, (int32_t)1, (float).8750, (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1, tmp83, tmp85, tmp87, inputIndices, outputIndices);
...
TILERelu4( (int32_t)1,  (int32_t)32,  (int32_t)32,  (int32_t)128, (int32_t)1, (float).8750, tmp97, tmp99,  kScale, kDoExtractTruncate);
...
inputIndicesFilePath = "/path/tensor_3.txt";
outputIndicesFilePath = "/path/tensor_4.txt";
inputIndices = loadIndicesFromTxt(inputIndicesFilePath);
outputIndices = loadIndicesFromTxt(outputIndicesFilePath);

uint64_t* tmp113 = make_array<uint64_t>( (int32_t)1,  (int32_t)16,  (int32_t)16,  (int32_t)256);
TILEWrapper( (int32_t)1,  (int32_t)16,  (int32_t)16,  (int32_t)128,  (int32_t)3,  (int32_t)3,  (int32_t)256,  (int32_t)1, (float).8750, (int32_t)0, (float).9375, (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  tmp109, tmp111, tmp113, inputIndices, outputIndices);
...
TILERelu4( (int32_t)1,  (int32_t)16,  (int32_t)16,  (int32_t)256, (int32_t)0, (float).9375, tmp122, tmp124,  kScale, kDoExtractTruncate);
...
inputIndicesFilePath = "/path/tensor_4.txt";
inputIndices = loadIndicesFromTxt(inputIndicesFilePath);
outputIndices = {};

uint64_t* tmp132 = make_array<uint64_t>( (int32_t)1,  (int32_t)16,  (int32_t)16,  (int32_t)256);
TILEWrapper( (int32_t)1,  (int32_t)16,  (int32_t)16,  (int32_t)256,  (int32_t)3,  (int32_t)3,  (int32_t)256,  (int32_t)0, (float).9375, (int32_t)0, (float)1.0, (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1, tmp128, tmp130, tmp132, inputIndices, outputIndices);
...
TILERelu4( (int32_t)1,  (int32_t)16,  (int32_t)16,  (int32_t)256, (int32_t)0, (float)1.0, tmp141, tmp143,  kScale, kDoExtractTruncate);
...
// if apply ratio = 100 %, no necessary to load the position list.
inputIndices = {};

uint64_t* tmp151 = make_array<uint64_t>( (int32_t)1,  (int32_t)16,  (int32_t)16,  (int32_t)256);
TILEWrapper( (int32_t)1,  (int32_t)16,  (int32_t)16,  (int32_t)256,  (int32_t)3,  (int32_t)3,  (int32_t)256,  (int32_t)0, (float)1.0, (int32_t)1, (float)1.0, (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1, tmp147, tmp149, tmp151, inputIndices, outputIndices);
...
TILERelu4( (int32_t)1,  (int32_t)16,  (int32_t)16,  (int32_t)256, (int32_t)1, (float)1.0, tmp161, tmp163,  kScale, kDoExtractTruncate);
...
TILEWrapper( (int32_t)1,  (int32_t)16,  (int32_t)16,  (int32_t)256,  (int32_t)3,  (int32_t)3,  (int32_t)512,  (int32_t)1, (float)1.0, (int32_t)0, (float)1.0, (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1, tmp167, tmp169, tmp171, inputIndices, outputIndices);
...
TILERelu4( (int32_t)1,  (int32_t)16,  (int32_t)16,  (int32_t)512, (int32_t)0, (float)1.0, tmp180, tmp182,  kScale, kDoExtractTruncate);
...
TILEWrapper( (int32_t)1,  (int32_t)16,  (int32_t)16,  (int32_t)512,  (int32_t)3,  (int32_t)3,  (int32_t)512,  (int32_t)0, (float)1.0, (int32_t)0, (float)1.0, (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  tmp186, tmp188, tmp190, inputIndices, outputIndices);
...
TILERelu4( (int32_t)1,  (int32_t)16,  (int32_t)16,  (int32_t)512, (int32_t)0, (float)1.0, tmp199, tmp201,  kScale, kDoExtractTruncate);
...
TILEWrapper( (int32_t)1,  (int32_t)16,  (int32_t)16,  (int32_t)512,  (int32_t)3,  (int32_t)3,  (int32_t)512,  (int32_t)0, (float)1.0, (int32_t)1, (float)1.0, (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  tmp205, tmp207, tmp209, inputIndices, outputIndices);
...
TILERelu4( (int32_t)1,  (int32_t)16,  (int32_t)16,  (int32_t)512, (int32_t)1, (float)1.0, tmp218, tmp220,  kScale, kDoExtractTruncate);
...
TILEWrapper( (int32_t)1,  (int32_t)8,  (int32_t)8,  (int32_t)512,  (int32_t)3,  (int32_t)3,  (int32_t)512,  (int32_t)1, (float)1.0, (int32_t)1, (float)1.0, (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  tmp230, tmp232, tmp234, inputIndices, outputIndices);
...
TILERelu4( (int32_t)1,  (int32_t)8,  (int32_t)8,  (int32_t)512, (float)1.0, (int32_t)1, tmp243, tmp245,  kScale, kDoExtractTruncate);
...
TILEWrapper( (int32_t)1,  (int32_t)8,  (int32_t)8,  (int32_t)512,  (int32_t)3,  (int32_t)3,  (int32_t)512, (int32_t)1, (float)1.0, (int32_t)1, (float)1.0, (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1, tmp249, tmp251, tmp253, inputIndices, outputIndices);
...
TILERelu4( (int32_t)1,  (int32_t)8,  (int32_t)8,  (int32_t)512, (int32_t)1, (float)1.0, tmp262, tmp264,  kScale, kDoExtractTruncate);
...
TILEWrapper( (int32_t)1,  (int32_t)8,  (int32_t)8,  (int32_t)512,  (int32_t)3,  (int32_t)3,  (int32_t)512,  (int32_t)1, (float)1.0, (int32_t)0, (float)0.0, (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1,  (int32_t)1, tmp268, tmp270, tmp272, inputIndices, outputIndices);
