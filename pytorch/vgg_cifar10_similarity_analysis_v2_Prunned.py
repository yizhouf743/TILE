...
import os, copy, sys
# import model.vgg_cifar10_prunned as vgg
import model.vgg_cifar10_recon_prunned as vgg
...
def approximate_transfer(net):
  ...
  for name in key:
    if all([x in name for x in ['features', 'conv', 'weight']]):
      ...
      # print('transfered shape for ', name, ' is:',  torch.count_nonzero(count_np_row).item(), "x", torch.count_nonzero(count_np_col).item())
      # # uncount inp_chans for each out_chans:
      # remain_ops = torch.count_nonzero(count_np_row) * torch.count_nonzero(count_np_col)
      # prunned_item = remain_ops - count_np_col.sum()
      # print('% of Prunned ops for the layer after approximate', 100 * (prunned_item / remain_ops).item(), ' %')
      # remain_ops = torch.count_nonzero(count_np_row) * torch.count_nonzero(count_np_col)
      # prunned_item = remain_ops - count_np_col.sum()
      # print('#. of Remain ops for the layer after approximate: ', torch.ceil(count_np_col.sum() / c_n).to(torch.int32))
      # print('#. of Prunned ops for the layer after approximate: ', torch.floor(prunned_item / c_n).to(torch.int32))
      # # print(c_n)
      counter += 1
...
mask = torch.load(save_location + sub_location + 'finetuned_mask.pth', map_location=torch.device(device))
pre_trained_model = torch.load(save_location + sub_location + 'rename_para.pth', map_location=torch.device(device))
...
if __name__ == '__main__':
    ...
    while (acc < guilde_acc) and (thres >= -2):
      ...
      if acc >= guilde_acc:
        torch.save(partial_order, save_location + sub_location + 'Partial_Model_no_loss_' + str(thres) +'_v4_mask.pth')
        torch.save(net.state_dict(), save_location + sub_location + 'Partial_Model_no_loss_' + str(thres) +'_v4.pth')
        print('save parameter path: ', save_location + sub_location + 'Partial_Model_no_loss_' + str(thres) +'_v4.pth')
