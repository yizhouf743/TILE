    ...
    for name in layer_key:
      if 'conv.weight' in name:
        if 'features.0.conv.weight' not in name:
          ...
          flatten_cal_grad = cal_subgrad(flatten_x, flatten_grad_f, lmbda)
          flatten_cal_grad = get_momentum_grad(grad_dict, name, momentum, flatten_cal_grad)      
          # compute trial iterate
          flatten_hat_x = grad_descent_update(flatten_x, lr, flatten_cal_grad)
